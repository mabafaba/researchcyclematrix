...
rcm %>% filter(unit=="data") %>%
  ...
  mutate(comment = gsub("\n","<br>",comment)) %>%
  dplyr::arrange(date.hqsubmission.actual,rcid,status)%>% group_by(status) %>% summarise(count = n()) %>%( knitr::kable)
...
rcm %>% filter(unit=="data") %>%
  ...
  dplyr::select(rcid,file.id,comment,date.hqsubmission.actual,status) %>%
  mutate(comment = gsub("\n","<br>",comment)) %>%
  dplyr::arrange(date.hqsubmission.actual,rcid,status) %>% knitr::kable(format = "html",escape = FALSE) %>% kableExtra::kable_styling()
