...
cat("\n\n### Validated in the last two weeks")

recently_validated %>% select(rcid,file.id,comment,date.validated) %>% arrange(date.validated,rcid) %>%  knitr::kable(format = "markdown")


cat("\n\n### With HQ / Under Review")

rcm %>% filter(grepl("hq|partner|field",status,ignore.case = T)) %>%
                 select(rcid,file.id,comment,date.hqsubmission.actual,status) %>%
                 arrange(status,date.hqsubmission.actual,rcid) %>% 
...
cat(paste("\n\n### Recently Validated (2 weeks)","\n\n"))
  ...
  kable(recently_validated_cl[[country]] %>% select(rcid,file.id,comment,days_since_validation) %>% arrange(days_since_validation),format = "markdown") %>% print
...
under_review<-under_review %>% select(rcid,file.id,date.hqsubmission.actual) %>% arrange(lubridate::ymd(date.hqsubmission.actual))
